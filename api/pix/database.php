...
function atualizarStatusPagamento($paymentId, $status, $dataAprovacao = null) {
    global $pdo;
    try {
        // BUG CORRIGIDO: WHERE payment_id = ? estava faltando no primeiro branch
        if ($status === 'approved' && $dataAprovacao) {
            ...
            // Qualquer outro status (cancelled, rejected, refunded…)
            $sql    = "UPDATE pagamentos_pix SET status = ?, data_atualizacao = NOW() WHERE payment_id = ?";
            $params = [$status, $paymentId];
        }
...
function listarPagamentosPorCliente($clienteEmail, $limit = 10) {
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            LIMIT ?
        ");
        $stmt->execute([$clienteEmail, (int)$limit]);
...
function estatisticasPagamentos() {
    ...
    try {
        ...
        $stmt = $pdo->query("SELECT status, COUNT(*) as quantidade, SUM(valor) as valor_total FROM pagamentos_pix GROUP BY status");
        $stats['por_status'] = $stmt->fetchAll();

        $stmt = $pdo->query("SELECT COUNT(*) as hoje, SUM(valor) as valor_hoje FROM pagamentos_pix WHERE DATE(data_criacao) = CURDATE()");
        $stats['hoje'] = $stmt->fetch();

        $stmt = $pdo->query("SELECT COUNT(*) as mes, SUM(valor) as valor_mes FROM pagamentos_pix WHERE status = 'approved' AND MONTH(data_aprovacao) = MONTH(CURDATE()) AND YEAR(data_aprovacao) = YEAR(CURDATE())");
